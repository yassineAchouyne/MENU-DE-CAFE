        ...
        <table class="table table-striped">
            ...
            ?>
                <tr>
                    ...
                    <td><?=$val['prix']?></td>
                    <td>
                        <a class="btn btn-outline-primary" href="menu.php?id=<?=$val['id']?>">Modifier</a>
                        <a class="btn btn-outline-danger" href="menu.php?sup=<?=$val['id']?>">Supprimer</a>
                    </td>
                </tr>
            <?php
            }
 ...
 ?>
   <article class="admin change">
    <form action="" method="post">
            ...
            <input type="number" name="prix" id="" value="<?=$tab['prix']?>" class="form-control" required>
...
if (isset($_POST['Ajouter'])) {
    $sql = $db->prepare("INSERT into menu(nom,prix) values (?,?)");
    $sql->execute([$_POST['nom'],$_POST['prix']]);
    header("Location:menu.php");
}
if (isset($_POST['Modifier'])) {
    $sql = $db->prepare("UPDATE `menu` SET`nom`=?, prix=? WHERE id=?");
    $sql->execute([$_POST['nom'],$_POST['prix'],$_GET['id']]);
    header("Location:menu.php");
}
if (isset($_GET['sup'])) {
    $sql = $db->prepare("DELETE FROM `menu` WHERE id=?");
